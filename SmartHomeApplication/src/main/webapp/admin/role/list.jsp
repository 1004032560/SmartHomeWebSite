...
<HTML>
	<HEAD>
		...
		<script type="text/javascript">
			function del(id){
				if(confirm("你确认删除吗?")){
				    window.location.href = "${pageContext.request.contextPath }/adminRole/del?id="+id;
				}
			}
			function add(){
				window.location.href = "${pageContext.request.contextPath}/admin/role/add.jsp";
			}
		
			function fenye(curPage){
				var name=$("#name").val();
				window.location.href = "${pageContext.request.contextPath}/adminRole/list?name="+name+"&curPage="+curPage;
			}
			$(function (){
				$("#tb").hide();
			});
			function assignPriv(id){
				$.post(
				   "${pageContext.request.contextPath}/adminPrivilege/selPrivByRid",
				   {id:id},
				   function(obj){
					   var row="";
					   $("#tb tr:gt(0)").remove();
					  for(var i in obj.allList){
						  ...
						  row+="<tr><td align='center'><input type='checkbox' "+tmp+" class='ck' value='"+d.id+"'/></td><td>"+d.id+"</td><td>"+d.name+"</td><td>"+d.module+"</td><td>"+d.value+"</td></tr>";
					  }
			...
			function save(id){
				...
				if(ids.length>1){
					ids=ids.substr(1);
					$.post(
					   "${pageContext.request.contextPath}/adminPrivilege/saveRole",
					   {id:id,ids:ids},
					   function(obj){
						   alert(obj?"ok":"err");
						   location.href="${pageContext.request.contextPath}/adminRole/list";
					   }
					);
				}else{
					alert("为啥不选啊");
									...
									<c:forEach items="${page.list}" var="entry" varStatus="status">
										<tr onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" style="HEIGHT: 22px">
												<button onclick="assignPriv(${entry.id})">分配权限</button>
											</td>
											<td align="center" style="HEIGHT: 22px">
												<a href="${ pageContext.request.contextPath }/adminRole/getById?id=${entry.id}">
